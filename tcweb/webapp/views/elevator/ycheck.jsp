  ...
  */
  ...
  //  $('#map_Y').attr("value",plat);
	...
	grid=$('#yctt').datagrid({
	    ...
	    url:'/tcweb/elevator/ychecklist',
	    queryParams:{},
	    columns:[[
	        {field:'registNumber',align:'center',halign:'center',title:'电梯编号',width:$(this).width() * 0.125,formatter: function(value,rec,index) {
	        ...
	        {field:'address',align:'left',halign:'center',title:'地址',width:$(this).width() * 0.125},
	        {field:'buildingName',align:'left',halign:'center',title:'楼盘',width:$(this).width() * 0.125},
	        {field:'area',align:'center',title:'区域',width:$(this).width() * 0.125},
	        {field:'jdbCompanyName',align:'left',halign:'center',title:'街道办',width:$(this).width() * 0.125},
	        {field:'jyCompanyName',align:'center',title:'检验单位',width:$(this).width() * 0.125},
	        {field:'inspector',align:'left',halign:'center',title:'检验人',width:$(this).width() * 0.125},
	        {field:'inspectDate',align:'center',title:'检验日期',width:$(this).width() * 0.125},
	        {field:'nextInspectDate',align:'center',title:'下次检验日期',width:$(this).width() * 0.125},
	        {field:'checkCategory',align:'center',title:'检验类别',width:$(this).width() * 0.125},
	        {field:'checkResult',align:'center',title:'检验结论',width:$(this).width() * 0.125},
	        {field:'checkReportNum',align:'left',halign:'center',title:'检验报告编号',width:$(this).width() * 0.125}
	     //   {field:'zjCompanyName',title:'质监局',width:150}  
	    ]],
	    pagination:true,
	    singleSelect:true
	});	
	$('#yctt').datagrid('getPager').pagination({displayMsg:'显示 {from} 至 {to} 条  共 {total} 条记录', afterPageText:'/{pages}', beforePageText:'页',pageList:[15,25,30,35,40]});  
...
function clearQuery(){
	...
	$("#qstartTime2").datebox("setValue","");  
	$("#qendTime2").datebox("setValue","");  
	 <%if(role !=10 && role != 11){ %>
...
function query(){  
	...
	var qstartTime2=$('#qstartTime2').datebox("getValue");  
	var qendTime2=$('#qendTime2').datebox("getValue"); 
	var area = "";
	  <%if(role !=10 && role != 11){ %>
    ...
    if(qtownshipStreets==""){
     ...
     grid.datagrid("options").url='/tcweb/elevator/ycquery';
     grid.datagrid("options").queryParams={'registNumber':registNumber,'buildingName':buildingName,'address':address,'area':area,'qstartTime':qstartTime,'qendTime':qendTime,'qstartTime2':qstartTime2,'qendTime2':qendTime2,'townshipStreets':qtownshipStreets};
     
    $('#yctt').datagrid('reload');
	}
...
<div region="north" style="overflow:hidden;background-color:rgb(201,220,245);">  
 ...
 <td><select id="qtownshipStreets"  class="easyui-combobox" name="qtownshipStreets" style="height:25px;"></select></td>
  ...
  <tr>
   <td  nowrap align="right">检验日期起：</td>
   <td><input id="qstartTime"  type="text" class="easyui-datebox" name="qstartTime" style="height:25px;" data-options="editable:false"></input></td>
   <td align="right" nowrap>止：</td>
   <td><input id="qendTime"  type="text" class="easyui-datebox" name="qendTime"  style="height:25px;" data-options="editable:false"></input></td>
   <td  nowrap align="right">下次检验日期起：</td>
   <td><input id="qstartTime2"  type="text" class="easyui-datebox" name="qstartTime2" style="height:25px;" data-options="editable:false"></input></td>
   <td align="right" nowrap>止：</td>
   <td><input id="qendTime2"  type="text" class="easyui-datebox" name="qendTime2" style="height:25px;" data-options="editable:false"></input></td>
   <td>
