...
$(function(){
	...
	$('#versionCode').combobox({
    	url:'/tcweb/user/versionCodeList',
  ...
  //	$('#btn-save,#btn-cancel').linkbutton(); 
	...
	grid=$('#tt').datagrid({
	    ...
	    columns:[[
	        ...
	        {field:'idCard',align:'center',halign:'center',title:'身份证',width:$(this).width() * 0.11},
	        {field:'companyName',align:'left',halign:'center',title:'单位名称',width:$(this).width() * 0.25},
	        ...
	        {field:'telephoneType',align:'center',halign:'center',title:'操作系统',width:100},
	        {field:'versionCode',align:'center',halign:'center',title:'软件版本',width:100},
	        {field:'type',align:'center',title:'角色',width:100,formatter: function(value,rec,index) {
		        var userId =rec.id;
...
function clearQuery(){
	...
	$('#telephoneType option:first').attr('selected','selected');
...
function query(){  
	 ...
	 var telephoneType =$('#telephoneType option:selected').val();
	 var versionCode  =$('#versionCode').combobox('getValue');
	 
     grid.datagrid("options").url='/tcweb/user/applistquery';
     grid.datagrid("options").queryParams={'loginName':loginName,'userName':userName,'companyName':companyName,'telephoneType':telephoneType,'versionCode':versionCode};
...
function saveUser(){ 
	...
	//	var binding =$('#binding').combobox('getValue');
		...
		var qualificationvalidate =$('#qualificationvalidate').datetimebox('getValue'); 
		if(!loginName){
...
<div region="north" style="overflow:hidden;;background-color:rgb(201,220,245);height:100px;">  
 <fieldset id="addDiv" style="width: 100%;height:100%;margin:0px;border: 1px solid #61B5CF;"><legend style="color:#0E2D5F;font-size:12px;font-weight: bold;">查询条件</legend>
    ...
    <table id="sousuo" style="border-collapse:separate; border-spacing:0px 5px;"> 
     <tr> 
...
<td>
   ...
   <tr>
   <td align="right" nowrap>操作系统：</td> 
   <td> 
   <select id="telephoneType"   name="telephoneType" style="width:220px;height:25px;padding-left: 15px;">
    <option value=""></option>
   <option value="android">android</option>
   <option value="ios">ios</option>
   </select>
   </td>
    <td align="right" nowrap>版本号：</td>
   <td> 
  <select id="versionCode"  class="easyui-combobox" name="versionCode" style="width:220px;height:25px;"></select>  
 </td>
  <td></td>
  <td></td>
 <td nowrap colspan="4">
...
<div id="user-window" title="详细信息" style="width:600px;height:370px;"> 
<div class="easyui-layout" data-options="fit:true">  
  <form method="post"> 
  <div data-options="region:'north'" style="height:250px;text-align:center;" >   
  <table  style="width:100%;margin:auto">    
   <tr>      
   <td width="150" align="center" style="background-color:#ffffcc;">登陆名</td>      
   <td><input id="loginName" name="loginName" class="easyui-validatebox" data-options="required:true,validType:'length[1,30]'"></input></td>  
    <td width="150" align="center" style="background-color:#ffffcc;">密码</td>      
   <td><input id="password" name="password" type="password" class="easyui-validatebox" data-options="required:true,validType:'length[1,15]'"></input></td> 
   </tr>
   <tr> 
   <td align="center" style="background-color:#ffffcc;">用户名</td>      
   <td><input id="userName" name="userName" class="easyui-validatebox" data-options="required:true,validType:'length[1,15]'"></input></td> 
   <td align="center" style="background-color:#ffffcc;">身份证号</td>      
   <td><input id="idCard" name="idCard"></input></td> 
    </tr>
   <tr> 
    <td align="center" style="background-color:#ffffcc;">联系电话</td>      
   <td><input id="contactPhone" name="contactPhone"></input></td>
   <td align="center" style="background-color:#ffffcc;">移动电话</td>      
   <td><input id="telephonemobile" name="telephonemobile"></input></td>
   </tr>
  
    <tr>  
          
<td align="center" style="background-color:#ffffcc;">单位类型</td>  
 <%if(role==10 || role==11){%> 
 <td><select id="companyType"   name="companyType" style="width:130px;" onchange="getzjAndjdbCompanyList()"> 
 <option value="质监">质监</option> 
 <option value="街道办">街道办</option>   
 </select></td>
 <%}
 else if (role==22 || role==23){%>
 <td><select id="companyType"   name="companyType" style="width:130px;" onchange="getzjCompanyList()">
 <option value="质监">质监</option>   
 ...
 <td><select id="companyType"   name="companyType" style="width:130px;" onchange="secAutoComplete()"> 
...
<%}
 ...
 <td><select id="companyType"   name="companyType" style="width:130px;" onchange="secAutoComplete()"> 
    ...
    <%}%> 
    <td align="center" style="background-color:#ffffcc;">所在单位</td> 
  <%if(role==10 || role==11){%>
      <td><select id="companyId" name="companyId" class="easyui-combobox" style="width:132px;"></select></td>
    <%}
  else  if(role==22 || role==23){ %>
 <td><select id="companyId" name="companyId" class="easyui-combobox" style="width:132px;"></select></td>
 <%}  
  else {%>
   <td><input id="companyId2"  name="companyId2" style="background-color:#87CEEB;width:130px;"></input>
   <input id="companyId" name="companyId" type="hidden"></input>
   </td>
   <%}%>
   </tr>
    <tr> 
     <td align="center" style="background-color:#ffffcc;">类型</td>      
  <!--  <td><input id="qy_nameinfo" name="qy_name"></input><span class="fontShow"><font color="red">*</font></span></td>   -->  
   ...
   <td><select id="type"  name="type" style="width:130px;">
    ...
    <option value="员工">员工</option>    
      ...
      <%}%>
 <%if(ispcsuper == 1){ %> 
   <td align="center" style="background-color:#ffffcc;">3D扫描</td>
<td>
 <select id="threedscanning"  name="threedscanning" style="width:132px;">
  ...
  <%}%>  
    ...
    <tr>
    <td align="center" style="background-color:#ffffcc;">维保人员资质作业种类</td>      
   <td><input id="speEquQualification" name="speEquQualification"></input></td> 
   <td align="center" style="background-color:#ffffcc;">资质有效期</td>      
   <td><input id="qualificationvalidate" name="qualificationvalidate" class="easyui-datebox" data-options="editable:false"></input></td> 
    </tr>
     <tr>
     <td align="center" style="background-color:#ffffcc;">注册日期</td>      
   <td><input id="qregistereddate" name="qregistereddate" class="easyui-datebox" data-options="editable:false"></input></td>
   <td align="center" style="background-color:#ffffcc;">电话绑定状态</td> 
   <td title="解除绑定"><select id="binding" name="binding"  class="selector" style="width:130px;" disabled="disabled"/>
  ...
  <!--   <a href="javascript:void(0)" onclick="saveUser()" id="btn-save" icon="icon-save">保存</a>  
     <a href="javascript:void(0)" onclick="closeWindow()" id="btn-cancel" icon="icon-cancel">取消</a>   --> 
      <a href="#" class="easyui-linkbutton" icon="e-icon fa fa-save fa-lg" onclick="saveUser()" style="width:100px">保存</a>
   ...
   2.“单位类型”一栏，市质监对应成都市质监局，质监对应各个区县质监局<br>
