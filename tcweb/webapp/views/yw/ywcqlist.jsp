...
$(function(){
	...
	var inputWidth = $('#registNumber').width(); 
	 $("#isProcessFlag").css("width", inputWidth);
   ...
   //    url: '/tcweb/elevator/getjdbCompanyListByarea?companyArea='+encodeURI(record.value),
    ...
    <% }%>
	grid=$('#ywcqtt').datagrid({
	    title:'维保超期列表',
	    fitColumns:true,
 	    striped:true,
	    pageSize:25,
	    pageList:[15,25,30,35,40],
	    url:'/tcweb/yw/ywcqlist',
	    queryParams:{},
	    columns:[[
	        {field:'registNumber',align:'center',halign:'center',title:'电梯编号',width:$(this).width() * 0.071,formatter: function(value,rec,index) {
	        ...
	        {field:'address',align:'left',halign:'center',title:'地址',width:$(this).width() * 0.071},
	        {field:'buildingName',align:'left',halign:'center',title:'楼盘名称',width:$(this).width() * 0.071},
	        {field:'companyName',align:'left',halign:'center',title:'维保公司',width:$(this).width() * 0.071},
	        {field:'area',align:'center',title:'行政区划',width:$(this).width() * 0.071},
	        {field:'jdbCompanyName',align:'left',halign:'center',title:'乡镇街道办',width:$(this).width() * 0.071},
	        {field:'userName',align:'center',halign:'center',title:'维保人员',width:$(this).width() * 0.071},
	        {field:'telephonemobile',align:'left',halign:'center',title:'维保人员手机',width:$(this).width() * 0.071},
	        {field:'endTime',align:'center',halign:'center',title:'上一次维保结束时间',width:$(this).width() * 0.071},
	        {field:'properWhTime',align:'center',halign:'center',title:'维保最后期限',width:$(this).width() * 0.071,formatter: function(value,rec,index) {
                if(value!="")
                    return value.substring(0,10);
		         }},
		    {field:'beyondTime',align:'center',title:'超期时间(天)',width:$(this).width() * 0.071}, 
		    {field:'beyondLevel',align:'center',title:'超期级别',width:$(this).width() * 0.071},
	        {field:'isProcessFlag',align:'center',title:'超期是否处理',formatter: function(value,rec,index) {
                 if(value==0)
                     return "未处理";
                 else
                     return "已处理";
		         }},
		     {field:'dealWithTime',align:'center',halign:'center',title:'超期处理时间',width:$(this).width() * 0.071,formatter: function(value,rec,index) {
		                if(value!="")
		                    return value.substring(0,10);;
				         }}, 	    
	    ]],
	...
	$('#ywcqtt').datagrid('getPager').pagination({displayMsg:'显示 {from} 至 {to} 条  共 {total} 条记录', afterPageText:'/{pages}', beforePageText:'页',pageList:[15,25,30,35,40]});  
...
//	$('#isProcessFlag').combobox('select','2');
$('#isProcessFlag option:first').attr('selected','selected');
...
//	var isProcessFlag =$('#isProcessFlag').combobox('getValue'); 
    var  isProcessFlag= $('#isProcessFlag option:selected').val();
    ...
    if(qtownshipStreets==""){
     ...
     grid.datagrid("options").url='/tcweb/yw/cqquery';
     grid.datagrid("options").queryParams={'registNumber':registNumber,'isProcessFlag':isProcessFlag,'companyId':ywCompanyId,'qstartTime':qstartTime,'qendTime':qendTime,'area':area,'townshipStreets':qtownshipStreets};
     
    $('#ywcqtt').datagrid('reload');
	}
...
<div region="north" style="overflow:hidden;background-color:rgb(201,220,245);">  
 <fieldset id="addDiv" style="margin:0px;border: 1px solid #61B5CF;"><legend style="#c8d9f5;font-weight:bold">查询条件</legend>
   ...
   <td> <select id="isProcessFlag"   name="is_Process" style="height:25px;">
