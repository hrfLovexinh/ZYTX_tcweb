...
function compute1(qstartTime,qendTime){
	var ydata = new Array();
	var ydata2 = new Array();
	var ydata3 = new Array();
	var ydata4 = new Array();
	jQuery.post('/tcweb/yw/ywshenhetj',{'qstartTime':qstartTime,'qendTime':qendTime}, function(data){ 
        data = eval(data);//POST方法必加，ajax方法自动处理了      
        ydata.push(data.ywfwmobileCount);
        ydata.push(data.ywCount-data.ywfwmobileCount-data.ywtempfwCount);

        /*    
        ydata2.push(data.ywundoCount); 
        ydata2.push(data.ywtgCount);
        ydata2.push(data.ywuntgCount);
        ydata2.push(data.ywnoimageCount);
        */
        ydata2.push(data.ywmobileshCount); 
        ydata2.push(data.ywautoshCount);
        ydata2.push(data.ywptsheCount);

        ydata3.push(data.ywpcyxCount); 
        ydata3.push(data.ywpcwxCount); 
        ydata3.push(data.ywpcysCount); 
        ydata3.push(data.ywpcnoiCount); 
        ydata3.push(data.ywimageqxCount); 
        ydata3.push(data.ywwffbdataCount); 
        ydata3.push(data.ywzddataCount); 
        ydata3.push(data.ywnoimagedataCount); 

        ydata4.push(data.ywpcnoyxCount); 
        ydata4.push(data.ywpcnowxCount); 
        ydata4.push(data.ywpcnoysCount); 
        ydata4.push(data.ywpcnonoiCount); 
        ydata4.push(data.ywgiswrongCount); 
        ydata4.push(data.ywfeixianchangCount); 
        ydata4.push(data.ywzhifuyinCount); 
        ydata4.push(data.ywelefuyinCount); 
        
       
        draw1(ydata,ydata2,ydata3,ydata4); 
	   }, 'json');
}

function draw1(ydata,ydata2,ydata3,ydata4){
	 $('#ywshtj').height($('#centerDiv').height()* 0.95);
	 var myChart = echarts.init(document.getElementById('ywshtj'));  
	...
	var xAxisData=['前端防伪开', '前端防伪关'];
	var xAxisData2=['未处理','通过','无效','无图片'];
	var xAxisData3=['有效图片','无效图片','疑似图片','无图片','图像清晰正常', '无法分辨数据', '遮挡数据', '无图像数据'];
	var xAxisData4=['有效图片','无效图片','疑似图片','无图片','GPS位置错误', '非现场运维数据', '纸质复印件数据', '电子复印件数据'];
	    ...
	    series: [
	     {
	         ...
	         name:'是否需要处理',           
	         data:[ 
	  	     {value:ydata[0], name:'不需要处理'}, 
             {value:ydata[1], name:'需要处理'}
             ], 
             itemStyle:{ 
             normal:{ 
               label:{ 
        ...
        },
	        {
	            ...
	            name:'处理数量',           
	            data:[ 
	  	        {value:ydata2[0], name:'前端处理'}, 
                {value:ydata2[1], name:'后端自动处理'}, 
                {value:ydata2[2], name:'平台人工处理'}
                ], 
                itemStyle:{ 
                normal:{ 
                  label:{ 
	...
	setInterval(function(){
	    ...
	    option.series[2].data.push(parseInt(Math.random()*100));
	    option.series[3].data.push(parseInt(Math.random()*100));
...
function query(){
	...
	if(validateTime(qstartTime,qendTime)){
	    compute1(qstartTime,qendTime);
	}
	else{
...
function validateTime(qstartTime,qendTime){
	...
	if(qstartTime == '' &&  qendTime == ''){
	...
	else if(qstartTime != '' &&  qendTime != ''){
		var d1 = qstartTime.replace(/\-/g, "/");
		var h1 = new Date(d1);

		var d2 = qendTime.replace(/\-/g, "/");
		var h2 = new Date(d2);

		if(h2.getTime() - h1.getTime() < 0){
			$.messager.alert('操作失败', '开始时间不能大于结束时间，请重新选择日期范围', 'error');
			return false;

			}
		
		var dayNumber = parseInt((h2.getTime() - h1.getTime()) / parseInt(timeType));
		if(dayNumber > 16){
			$.messager.alert('操作失败', '查询时间间隔不能大于16天,请重新选择查询日期范围', 'error');
			return false;
		}
		else{
			return true;
		...
		$.messager.alert('操作失败', '请选择查询日期范围的起止日期', 'error');
		return false;
...
<body class="easyui-layout" data-options="fit:true">
 ...
 <fieldset id="addDiv" style="margin-left:0px;border: 1px solid #61B5CF;"><legend style="#c8d9f5;font-weight:bold">查询条件</legend>
   ...
   <td><input id="qstartTime"  type="text" class="easyui-datebox" style="width:220px" name="qstartTime" style="height:25px;"></input></td>
   <td align="right" nowrap>结束时间：</td>
   <td><input id="qendTime"  type="text" class="easyui-datebox"  style="width:220px" name="qendTime" style="height:25px;"></input></td>
    <td>
