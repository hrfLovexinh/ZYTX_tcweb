...
public class ElevatorController extends ApplicationController{  
	 ...
	 public View elevatorlistByOrder(int page, int rows,String sort,String order){		
		     ...
		     String sql ="";
			 try {
		     if(role==2 || role ==1){ //系统管理员 TwoCodeCompanyInfo 
		    	 
				 total  =ElevaltorInfoNewVO.count(ElevaltorInfoNewVO.class, null, null); 
				 if("1".equals(cityName)){ 
				  sql ="select t.jyjyFlag, t.shibieCode,t.shemiFlag,t.dailingFlag,t.id,t.area,t.registNumber,isnull(t.address,'') as address,isnull(t.buildingName,'') as buildingName,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,t.zzCompanyId,t.azCompanyId,t.wgCompanyId,t.ywCompanyId,t.zjCompanyId,t.jyCompanyId,t.townshipStreets,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,y.endTime as subTime,  isnull(tcr.deviceId,'') as deviceId,t.ischangInfo,t.usePlace from TwoCodeElevatorInfo  t left join TwoCodeDeviceRelationInfo tcr on t.registNumber = tcr.registNumber left join YwManagerInfo  y  on t.registNumber = y.registNumber  ";			 
				 }
				 else{
			...
			//		 sql ="select t.jyjyFlag,t.shemiFlag,t.dailingFlag,t.id,t.area,t.registNumber,isnull(t.address,'') as address,isnull(t.buildingName,'') as buildingName from TwoCodeElevatorInfo  t left join TwoCodeDeviceRelationInfo tcr on t.registNumber = tcr.registNumber left join YwManagerInfo  y  on t.registNumber = y.registNumber  ";
		     ...
		     if(role==22 || role ==23){ //系统管理员 TwoCodeCompanyInfo  shizhijian
		    	 total  =ElevaltorInfoNewVO.count(ElevaltorInfoNewVO.class, null, null);
		    	 if("1".equals(cityName)){ 
	...
	//		  System.out.println(new JsonView(result));
	 ...
	 public View getCompanyList(int page, int rows) throws Exception {
		 ...
		 if(role==10 || role==11){
			 ...
			 items = CompanyInfo.findAll(CompanyInfo.class, "type like ?", new Object[] { "%质监%" });
		 }
		 else{
	...
	public View getWgCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "使用" });
		return new JsonView(items);
	}
	
	public View getYwCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName,ispasteyw from TwoCodeCompanyInfo where type = ? and ispasteyw = 0", new Object[] { "维保" });
		return new JsonView(items);
	}
	...
	//	 int userid = userinfo.getId();
		 ...
		 if(role == 30 || role == 31){
				//行业协会要找市质监的companyId
	    		companyinfo = CompanyInfoVO.findFirstBySql(CompanyInfoVO.class, "select id  from TwoCodeCompanyInfo where type = ?", new Object[]{"市质监"});
	    		if(companyinfo != null){
		 ...
		 if(role == 10 || role == 11){  //区县质监
			 ...
			 sql ="select isnull(te.ywCompanyId,0) as ywCompanyId, tt.id, tt.companyName,(case when  CHARINDEX('已考评',tt.companyName)>0 then 1 else 0 end) as indexOrder from (select  tc.id,(case when isnull(tr.ratingCompanyId,0) =0  then tc.companyName else  tc.companyName+'(已考评)' end) as companyName,ispasteyw from TwoCodeCompanyInfo tc left join (select ywCompanyID,ratingCompanyId from  TwoCodeYwQuaCredRating where  ratingDate = ? and ratingCompanyId =7   and ratingType =1) tr   on tc.id=tr.ywCompanyID where type = '维保' and ispasteyw = 0 ) tt  left join (select ywCompanyId from  TwoCodeElevatorInfo where area = ? group by ywCompanyId )te on tt.id = te.ywCompanyId   where te.ywCompanyId > 0 order by indexOrder desc ";
		     param = new Object[] {ratingDate,area,ratingCompanyId };
		 }
		 else{
			 sql = "select tt.id, tt.companyName,(case when  CHARINDEX('已考评',tt.companyName)>0 then 1 else 0 end) as indexOrder  from (select tc.id,(case when isnull(tr.ratingCompanyId,0) =0  then tc.companyName else  tc.companyName+'(已考评)' end) as companyName,ispasteyw from TwoCodeCompanyInfo tc left join (select ywCompanyID,ratingCompanyId from  TwoCodeYwQuaCredRating where  ratingDate = ? and ratingCompanyId =?   and ratingType =1) tr  on tc.id=tr.ywCompanyID where type = '维保' and ispasteyw = 0 ) tt order by indexOrder desc";
	...
	//	List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
	//			"select top 100 id,companyName from TwoCodeCompanyInfo where type = ? ", new Object[] { "维保" });
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
							"select  id,companyName from TwoCodeCompanyInfo where type = ? ", new Object[] { "维保" });
		return new JsonView(items);
	}
	
	public View getZzCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "制造" });
		return new JsonView(items);
	}
	...
	public View getAzCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "安装" });
		return new JsonView(items);
	}
	
	public View getJyCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "检验" });
		return new JsonView(items);
	}
	...
	public View getnewZjCompanyList(int page, int rows) throws Exception {
		...
		if(role==10 || role==11){
	         ...
	         int zjcompanyId = userInfoVO.getCompanyId();
	         items = CompanyInfo.findBySql(CompanyInfo.class,
	 				"select id,companyName from TwoCodeCompanyInfo where type = ? and id = ?", new Object[] { "质监",zjcompanyId});
	    	 }
		}else{
		 items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type like ?", new Object[] { "%质监%" });
		}
		return new JsonView(items);
	}
	...
	public View getZjCompanyList(int page, int rows) throws Exception {
		...
		if(role==10 || role==11){
		 ...
		 items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "质监" });
		}
		return new JsonView(items);
	}
	...
	public View getKpZjCompanyList(int page, int rows) throws Exception {
		...
		if(role==10 || role==11){
		 ...
		 items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where (type like ? or type = ? or type = ?) and companyName != ?", new Object[] { "%质监%","检验","行业协会","未知" });
		}
		return new JsonView(items);
	}
	...
	public View getzjAndjdbCompanyList(String companyType,int page, int rows) throws Exception {
		...
		if(role==10 || role==11){
	         ...
	         if("质监".equals(companyType)){
	         items = CompanyInfo.findBySql(CompanyInfo.class,
	 				"select id,companyName from TwoCodeCompanyInfo where type = ? and id = ?  ", new Object[] { "质监",zjcompanyId});
	    	 }
	         if("街道办".equals(companyType)){
	        	 items = CompanyInfo.findBySql(CompanyInfo.class,
	 	 				"select id,companyName from TwoCodeCompanyInfo where type = ? and parea = ? ", new Object[] { "街道办",area}); 
	          }
	         }
		}else{
	...
	public View getjdbCompanyList(int page, int rows) throws Exception {
		List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
				"select id,companyName from TwoCodeCompanyInfo where type = ?", new Object[] { "街道办" });
		return new JsonView(items);
	}
	...
	public View getjdbCompanyListByarea(String companyArea){
		...
		try {
			items = CompanyInfo.findBySql(CompanyInfo.class,
					"select id,companyName from TwoCodeCompanyInfo where type = ? and (parea = ? or companyName = ? )", new Object[] { "街道办",companyArea,"未知" });
		} catch (ActiveRecordException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public View getjdbCompanyListByarea2(){
		...
		try {
		 UserInfoVO userInfoVO = UserInfoVO.findFirstBySql(UserInfoVO.class, "select tc.area,tu.companyId from  TwoCodeUserExtInfo tu  left join TwoCodeCompanyInfo tc on tu.companyId = tc.id where  tu.userid = ? ", new Object[] { userid });
			...
			int zjcompanyId = userInfoVO.getCompanyId();
		    String companyArea = userInfoVO.getArea();
			items = CompanyInfo.findBySql(CompanyInfo.class,"select id,companyName from TwoCodeCompanyInfo where type = ? and (parea = ? or companyName = ? )", new Object[] { "街道办",companyArea,"未知" });
			}
		} catch (ActiveRecordException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View queryYwCompanyByReg(String registNumber) throws Exception {
	 ...
	 public String add(ElevaltorInfo elevaltorInfo){
		...
		//要程序更新标签信息表的updateTime字段，小廖那边要用
		...
		//判断registcode存在不存在
		 ...
		 CompanyInfo companyInfo = CompanyInfo.findFirstBySql(CompanyInfo.class, "select id from TwoCodeCompanyInfo where companyName like ? and type = '质监'", new Object[] { areaStr });
		 if(companyInfo != null)
			 elevaltorInfo.setZjCompanyId(companyInfo.getId());
		 
		 if("".equals(elevaltorInfo.getDeviceId())){  //不关联黑匣子设备
   ...
   //变更关系
		 ...
		 //       ActiveRecordBase.beginTransaction();
		        ...
		        if(oldElevaltorInfoVO.getYwCompanyId() != elevaltorInfo.getYwCompanyId()){ //运维公司ID不等说明运维公司有变动
		 ...
		 //          str ="维保单位变更:"+oldywCompanyName+"--->"+ywCompanyName+";";
		           changeFlag = true;
		       
		        }
		        //第0步物管变更关系
		        if(oldElevaltorInfoVO.getWgCompanyId() != elevaltorInfo.getWgCompanyId()){//物管公司ID不等说明物管公司有变动
		    ...
		    //    	str=str+"使用单位变更:"+odlwgCompanyName+"--->"+wgCompanyName+";";
		        	 changeFlag2 = true;
		        }
		        /*
		        if(changeFlag){ //发生过需要记录的变更
		        UserInfo user=(UserInfo)session.getAttribute("sessionAccount");
		        if(user!=null){
		        System.out.println("操作人员："+user.getLoginName());
				...
				if(twoCodeDeviceRelationInfo == null){  //没有关系存在，添加新的关系
			...
			if("".equals(elevaltorInfo.getMap_Y())){
			 ...
			 String  areaStr ="%"+elevaltorInfo.getArea()+"%";
			 CompanyInfo companyInfo = CompanyInfo.findFirstBySql(CompanyInfo.class, "select id from TwoCodeCompanyInfo where companyName like ? and type = '质监' and area ! ='' and area is not null ", new Object[] { areaStr });
			 if(companyInfo != null)
				 elevaltorInfo.setZjCompanyId(companyInfo.getId());
		    ...
		    if(changeFlag || changeFlag2){ //发生过需要记录的变更
		        UserInfo user=(UserInfo)session.getAttribute("sessionAccount");
		        if(user!=null){
		        System.out.println("操作人员："+user.getLoginName());
		        System.out.println(str.substring(0, str.length()-1));
	 ...
	 public String delete(ElevaltorInfoVO elevaltorInfoVO){
		 ...
		 try {
			...
			if(twoCodeDeviceRelationInfo != null){   //删除与黑匣子的关联关系
			...
			if(elevaltorInfo != null){
				...
				if(ElevaltorInfo.execute(sql, new Object[]{registNumber})>0 ){
					...
					DdElevaltorInfoVO.execute("delete from TwoCodeDdElevatorInfo where registNumber = ? ", new Object[]{registNumber});  //删除预录入数据库中标签信息
					
				}
				
				if(HisYwInfoVO.execute(ywsql2, new Object[]{registNumber})> 0){  //拷贝历史运维表信息成功，删除历史运维表中标签运维信息
					HisYwInfoVO.execute("delete from YwManagerInfo where registNumber = ? ", new Object[]{registNumber});
				}
				
				if(YwInfo.execute(ywsql, new Object[]{registNumber})> 0 ){ //拷贝运维表信息成功，删除运维表中标签运维信息
	 ...
	 */
	 ...
	 public String sdelete(SelevaltorInfoView selevaltorInfoView){
		 ...
		 try {
		    if(source == 1){
			    ...
			    if(selevaltorInfo != null){   //删除新建的停用电梯
				   selevaltorInfo.destroy();
				   result = "success";	
			    }
		    }
		    else{     //更新已经粘贴标签的停用电梯标志，让其为取消停用电梯
	 ...
	 public String tingYong(ElevaltorInfoVO elevaltorInfoVO){
		 ...
		 try {
			    ...
			    if(num >0){
			    	...
			    	if(elelog.save())
			    	  result = "success";
			  //  	jkdy("updateDeviceStatus","5",registNumber);   //内网不通知科鸿
	 ...
	 public String redotingYong(ElevaltorInfoVO elevaltorInfoVO){
		...
		/*	    	TsDeviceService server = new TsDeviceService();  
			...
			//    	jkdy("updateDeviceStatus","2",registNumber);  //内网不通知科鸿
	 ...
	 public String queryByOrderexportExcel(ElevaltorInfoVO info){
		 ...
		 int townshipStreets2 =0; //非查询用的，是登陆过程中系统后台根据role不同展示不同列表用的
	...
	try {      
      ...
      if(!"".equals(conditions)){
	    sql ="select isnull(t.area,'') as area,isnull(t.registNumber,'') as registNumber,isnull(t.address,'') as address,isnull(t.buildingName,'') as buildingName,isnull(t.registCode,'') as registCode,isnull(t.useNumber,'') as useNumber,isnull(t.nextInspectDate,'') as nextInspectDate,isnull((select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用'),'') as wgCompanyName, isnull((select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保'),'') as ywCompanyName,isnull((select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造'),'') as zzCompanyName, isnull((select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装'),'') as azCompanyName,isnull((select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办'),'') as jdbCompanyName,isnull(y.endTime,'') as  subTime from TwoCodeElevatorInfo  t left join YwManagerInfo  y  on t.registNumber = y.registNumber  where "+ conditions;
     ...
     long total =ElevaltorQueryExportVO.countBySql(ElevaltorQueryExportVO.class, conditionsSql, null); 
  	 ...
  	 String[] hearders = new String[] {"电梯编号","注册代码", "地址", "楼盘名称", "行政区划", "街道办", "单位内部编号","下次检验日期","使用单位","维保单位","制造单位","安装单位","运维时间"};//表头数组 
  	 ElevatorQueryExportExcel2<ElevaltorQueryExportVO> ex = new ElevatorQueryExportExcel2<ElevaltorQueryExportVO>(); 
  	 SimpleDateFormat timeFormat = new SimpleDateFormat("yyyyMMddHHmmss");  
	 String filename = "电梯列表_"+timeFormat.format(new Date())+".xlsx";  
  ...
  //这一行非常关键，否则在实际中有可能出现莫名其妙的问题！！！
	...
	//	 List<TwoCodeElevatorYwCompanyInfo> items=TwoCodeElevatorYwCompanyInfo.findBySql(TwoCodeElevatorYwCompanyInfo.class, sql, null, "t.updateTime desc", rows, (page-1)*rows);
	 ...
	 public View ychecklist(int page, int rows) throws Exception{
		 System.out.println("执行ychecklist方法");
	    ...
	    //	 items = ElevaltorInfoVO.findAll(ElevaltorInfoVO.class, null, null, null, rows, (page-1)*rows);
	     ...
	     if(role==10 || role ==11){ //质监局
	         ...
	         String sql ="select t.id,t.registNumber,t.address,isnull(t.buildingName,'') as buildingName,isnull(t.area,'') as area,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName  from TwoCodeElevatorInfo  t where t.zjCompanyId=?";
	     ...
	     if(role==20 || role ==21){ //街道办
	         ...
	         System.out.println("所属质监局标签运维数："+total);
	         String sql ="select t.id,t.registNumber,t.address,isnull(t.buildingName,'') as buildingName,isnull(t.area,'') as area,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName  from TwoCodeElevatorInfo  t where t.townshipStreets=?";
	  	   	 items = ElevaltorInfoVO.findBySql(ElevaltorInfoVO.class, sql, new Object[]{zjcompanyId}, "t.id desc", rows, (page-1)*rows);
	    	 }
	     }
	      Map<String, Object> result = new HashMap<String, Object>();
	 ...
	 public View ycquery(ElevaltorInfoVO info,int page, int rows)throws Exception{
		 ...
		 if(!"".equals(conditions)){
			  sql ="select t.id,t.registNumber,t.address,isnull(t.buildingName,'') as buildingName,isnull(t.area,'') as area,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName  from TwoCodeElevatorInfo  t where "+ conditions;
	 ...
	 public View ddelevatorlist(int page, int rows) throws Exception{
		 System.out.println("执行ddelevatorlist方法");
		 String cityName = GlobalFunction.cityName;
	     UserInfo userinfo=(UserInfo)session.getAttribute("sessionAccount");
		 int userid = userinfo.getId();
		 String userName =userinfo.getLoginName();
	     ...
	     if(role==10 || role ==11){
	    	 ...
	    	 System.out.println("所属质监局标签个数："+total);
	    	 if("1".equals(cityName)){
	 ...
	 public View rukuddelevatorlist(int page, int rows) throws Exception{
		 System.out.println("执行rukuddelevatorlist方法");
	     UserInfo userinfo=(UserInfo)session.getAttribute("sessionAccount");
		 int userid = userinfo.getId();
		 String userName =userinfo.getLoginName();
		 int  role = userinfo.getRole();
	   ...
	   // 	 String sql ="select t.rukubeizhu,t.arrageType,t.pasteTime,t.pastePersonID,t.pasteNote,tad.userName,CONVERT(CHAR(19), t.arrangeTime, 20) as arrangeTime2,t.subTime2 as subTime2,t.id,t.registNumber,t.address,t.buildingName,t.building,t.unit,t.recordSate,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,(select te.userName from twocodeuserExtinfo te where   te.userid =t.arrangePersonID) as arrangePersonName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,(select top 1 y.subTime from YwManagerInfo y  where y.registNumber=t.registNumber and ywkind='0' order by y.id desc) as subTime,t.ischangInfo,t.ruKuValid  from TwoCodeDdElevatorInfo  t left join TwoCodeDeviceRelationInfo tcr on t.registNumber = tcr.registNumber left join TwoCodeDdEleappendix tad on t.registNumber = tad.registNumber where t.recordSate =3";
	    ...
	    //	 String sql ="select t.rukubeizhu,t.arrageType,t.pasteTime,t.pastePersonID,t.pasteNote,tad.userName,CONVERT(CHAR(19), t.arrangeTime, 20) as arrangeTime2,t.subTime2 as subTime2,t.id,t.registNumber,t.address,t.buildingName,t.building,t.unit,t.recordSate,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,(select te.userName from twocodeuserExtinfo te where   te.userid =t.arrangePersonID) as arrangePersonName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,(select top 1 y.subTime from YwManagerInfo y  where y.registNumber=t.registNumber and ywkind='0' order by y.id desc) as subTime,t.ischangInfo,t.ruKuValid  from TwoCodeDdElevatorInfo  t left join TwoCodeDeviceRelationInfo tcr on t.registNumber = tcr.registNumber left join TwoCodeDdEleappendix tad on t.registNumber = tad.registNumber where t.zjCompanyId=? and t.recordSate =3";
	    	 String sql ="select t.rukubeizhu,t.arrageType,t.pasteTime,t.pastePersonID,t.pasteNote,t.mobileUploadbeizhu,t.arrangeTime as arrangeTime2,t.subTime2 as subTime2,t.id,t.registNumber,t.address,t.buildingName,t.building,t.unit,t.recordSate,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,(select te.userName from twocodeuserExtinfo te where   te.userid =t.arrangePersonID) as arrangePersonName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,t.ruKuValid  from TwoCodeDdElevatorInfo  t  where t.zjCompanyId=? and t.recordSate =3";
	 ...
	 public View hechaddelevatorlist(int page, int rows) throws Exception{
	   ...
	   // 	 String sql ="select t.subPersonID,isnull(t.mobileUploadbeizhu,'') as mobileUploadbeizhu,isnull(t.rukubeizhu,'') as rukubeizhu,t.arrageType,t.pasteTime,t.pastePersonID,t.pasteNote,tad.userName,CONVERT(CHAR(19), t.arrangeTime, 20) as arrangeTime2,t.subTime2 as subTime2,t.id,t.registNumber,t.address,t.buildingName,t.building,t.unit,t.recordSate,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,(select te.userName from twocodeuserExtinfo te where   te.userid =t.arrangePersonID) as arrangePersonName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select top 1 y.subTime from YwManagerInfo y  where y.registNumber=t.registNumber and ywkind='0' order by y.id desc) as subTime,isnull(tcr.deviceId,'') as deviceId,t.ischangInfo,t.ruKuValid  from TwoCodeDdElevatorInfo  t left join TwoCodeDeviceRelationInfo tcr on t.registNumber = tcr.registNumber left join TwoCodeDdEleappendix tad on t.registNumber = tad.registNumber where t.recordSate =4";
	    	 items = DdElevaltorInfoVO.findBySql(DdElevaltorInfoVO.class, sql, null, "t.ruKuValid,t.id desc", rows, (page-1)*rows);
   ...
   //变更关系
	...
	//	 DdElevaltorInfoVO oldElevaltorInfoVO =null;
		 ...
		 try {
		        ...
		        /* 待定管理不需要处理变更关系，因为还没有正式使用，可以随便修改
		        //第0步运维变更关系
		        ...
		        */
		        ...
		        if(ddAppendixEleInfo != null){  //附加关系表中有附加记录
	...
	//	 useNumber2 =info.getUseNumber2();
	 ...
	 public String dddelete(DdElevaltorInfoVO elevaltorInfoVO){
		 ...
		 try {
			...
			if(ddAppendixEleInfo != null){  //删除待定标签附加表信息
			...
			if(elevaltorInfo != null){   //删除二维码标签基本信息
				elevaltorInfo.destroy();
				result = "success";   
			}
			 ActiveRecordBase.commit();
	 ...
	 public View ddquery(DdElevaltorInfoVO info,int page, int rows)throws Exception{
		 ...
		 System.out.println("查询条件registNumber----"+info.getRegistNumber());
	...
	//	 List<DdTwoCodeElevatorYwCompanyInfo> items=DdTwoCodeElevatorYwCompanyInfo.findBySql(DdTwoCodeElevatorYwCompanyInfo.class, sql, param, null, rows, (page-1)*rows);
	 ...
	 public String ddrukucheck(int id){
		 String cityName = GlobalFunction.cityName;
		 String result = "操作失败";
		 System.out.println("要进行入库的行ID为----"+id);
		 DdElevaltorInfoVO ddElevaltorInfoVO = null;
		 ElevaltorInfoVO elevaltorInfoVO = null;
		 ...
		 try {
			...
			System.out.println("ddrukucheck---异常");
			e2.printStackTrace();
		}
		 System.out.println("ddrukucheck---3915---"+Arrays.toString(strArea));
	//	 String[] strArea = new String[]{"锦江","青羊","金牛","成华","武侯","高新","天府新","青白江","龙泉驿","新都","温江","双流","郫县","金堂","大邑","蒲江","新津","都江堰","崇州","邛崃","彭州","安仁","大丰","简阳"};
		 String registNumber="";
		 String address="";
		 String area="";
		 String eleType="";
		 ...
		 String registCode="";
		 int townshipStreets=0;
		 String[] streleType=new String[]{"曳引与强制驱动电梯","曳引驱动乘客电梯","曳引驱动载货电梯","强制驱动载货电梯","液压驱动电梯","液压乘客电梯","液压载货电梯","自动扶梯与自动人行道","自动扶梯","自动人行道","其它类型电梯","防爆电梯","消防员电梯","杂物电梯"};
		 String[] strinoutDoor=new String[]{"室内","室外"};
		 int zzCompanyId=0;
		 int wgCompanyId=0;
		 int jyCompanyId=0;
		 int azCompanyId=0;
		 ...
		 try {
			...
			if(elevaltorInfoVO != null){  //按照编号找，找到了，说明已经存在该编号记录，不能入库
				result = "编号已经存在，不能重复入库";
			...
			if(elevaltorInfoVO != null){  //按照四项地址，楼盘，单元，栋找，找到了，说明已经存在该地址记录，不能入库
				result = "地址，楼盘，单元，栋已经存在，不能重复入库";
				DdElevaltorInfoVO.updateAll(DdElevaltorInfoVO.class, "ruKuValid=?,rukubeizhu=?", new Object[] {2,result}, "id=?", new Object[] { id});
				return result;
			}
			...
			//以上条件都满足了，可以进行入库操作了
			ActiveRecordBase.beginTransaction();
			  if(!"0".equals(cityName)){   //复制了shibieCode
	 ...
	 public String ddhecha(int id){
		 String result = "操作失败";
		 System.out.println("要进行核查的行ID为----"+id);
		 if(id >0){
		 try {
	 ...
	 public View queryYwinfoByReg(String registNumber) throws Exception{
		 ...
		 ywInfoVO = YwInfoVO.findFirstBySql(YwInfoVO.class, sql, new Object[] { registNumber });
	      Map<String, Object> result = new HashMap<String, Object>();
	      if(ywInfoVO != null){
	      result.put("姓名", ywInfoVO.getUserName());
		  result.put("联系电话", ywInfoVO.getContactPhone());
	      }
	      else{
	    	  result.put("姓名", "");
			  result.put("联系电话","");  
	 ...
	 public View ddelevatortasklist(int page, int rows) throws Exception{
		 System.out.println("执行ddelevatortasklist方法");
	   //   long total = ImageVO.countBySql(ImageVO.class, "exec 按用户获取图片信息  ?", new Object[] { "test" });
	   //   long total =ImageVO.execute("exec 按用户获取图片信息 ?", new Object[] { "test" });
	  //     List<ImageVO>  items = ImageVO.findBySql(ImageVO.class, "exec 按用户获取图片信息  ?", new Object[] { "test" });   
	   //    List<ImageVO> items = ImageVO.findBySql(ImageVO.class, "select CARNUM as carnum,DEV_ID as dev_id from image_201105", null, null, rows, (page-1)*rows);
	...
	//		 conditionsSql=" recordSate =0  and address like  ?  and buildingName like ? and building like ? and unit like ? and ywCompanyId = ?";
	 ...
	 public View getpastePersonList(int page, int rows) throws Exception {
			List<TCUserInfoView> items = TCUserInfoView.findBySql(TCUserInfoView.class,
					"select * from TCUserInfo where type = ?", new Object[] { "粘贴" });
			return new JsonView(items);
		}
	...
	//	 return result;
	 ...
	 public View ddetaskstatisticlist(int page, int rows) throws Exception{
		 System.out.println("执行ddetaskstatisticlist方法");
		 String cityName = GlobalFunction.cityName;
		 UserInfo userinfo=(UserInfo)session.getAttribute("sessionAccount");
		 int userid = userinfo.getId();
		 String userName =userinfo.getLoginName();
	...
	//	 long total =DdTwoCodeElevatorYwCompanyInfo.count(DdTwoCodeElevatorYwCompanyInfo.class, conditionsSql, param);
	 ...
	 public View companyinfotishi(int id){
		 ...
		 if(id >0){
			 DdElevaltorInfoVO ddElevaltorInfoVO =null;
			 try {
				String sql ="select (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName from TwoCodeDdElevatorInfo t   where t.id = ? ";
				ddElevaltorInfoVO  =  DdElevaltorInfoVO.findFirstBySql(DdElevaltorInfoVO.class, sql, new Object[]{id});
				if(ddElevaltorInfoVO != null){
	...
	/*	 Object[] param=null;
	 ...
	 public View getAutoYwCompanyList4(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 10 id,companyName from TwoCodeCompanyInfo where (type = ? or type = ?)  and companyName like ?", new Object[] { "维保","检验", "%"+q+"%"});
	 ...
	 public View getAutoYwCompanyList5(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 50 id,companyName from TwoCodeCompanyInfo where type = ? and ispasteyw = 0 and companyName like ?", new Object[] { "维保", "%"+q+"%"});
	 ...
	 public View getAutoYwCompanyList3(String q,String qtype, int limit) throws Exception {
		 ...
		 List<CompanyInfo> items = null;
		   if("1".equals(qtype)){
			 items = CompanyInfo.findBySql(CompanyInfo.class,
					"select id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "维保", "%"+q+"%"});
		   }
		   else{  
			   items = CompanyInfo.findBySql(CompanyInfo.class,
						"select id,userName as companyName from TCUserInfo where type = ?  ", new Object[] { "粘贴"});
	 ...
	 public View getAutoYwCompanyList2(String q,String qtype, int limit) throws Exception {
		 ...
		 List<CompanyInfo> items = null;
		   if("1".equals(qtype)){
			 items = CompanyInfo.findBySql(CompanyInfo.class,
					"select id,companyName from TwoCodeCompanyInfo where type = ? and ispasteyw = 0 and companyName like ?", new Object[] { "维保", "%"+q+"%"});
		   }
		   else{  
			   items = CompanyInfo.findBySql(CompanyInfo.class,
						"select id,userName as companyName from TCUserInfo where type = ?  ", new Object[] { "粘贴"});
	 ...
	 public View getAutoZzCompanyList(String q, int limit) throws Exception {
		 	q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 10 id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "制造", "%"+q+"%"});
	 ...
	 public View getAutoJyCompanyList(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 10 id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "检验", "%"+q+"%"});
	 ...
	 public View getAutoAzCompanyList(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 10 id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "安装", "%"+q+"%"});
	 ...
	 public View getAutoZjCompanyList(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 10 id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "质监", "%"+q+"%"});
	 ...
	 public View getAutoJdbCompanyList(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "街道办", "%"+q+"%"});
	 ...
	 public View getNewAutoJdbCompanyList(String areaType,String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ? and parea = ?", new Object[] { "街道办", "%"+q+"%",areaType});
	 ...
	 public View getAutoWgCompanyList(String q, int limit) throws Exception {
		 q = new String(q.getBytes("ISO-8859-1"), "UTF-8");
			List<CompanyInfo> items = CompanyInfo.findBySql(CompanyInfo.class,
					"select top 100 id,companyName from TwoCodeCompanyInfo where type = ?  and companyName like ?", new Object[] { "使用", "%"+q+"%"});
	...
	//	 int userid = userinfo.getId();
		 ...
		 try {
			...
			if(user != null){
				...
				twoCodeLogInfo.setLogAction("查询");
				twoCodeLogInfo.setLogContext("电梯管理查询");
	 ...
	 public String putdailingele(int id){  
		 ...
		 try {  
			    ...
			    CompanyInfo	 zjCompanyInfo =CompanyInfo.findFirst(CompanyInfo.class, "companyName =? and type = ?", new Object[] {"","质监"});
	...
	//	 int userid = userinfo.getId();
		 ...
		 try {
			...
			if(user != null){
				...
				twoCodeLogInfo.setLogPerson(userName);
				twoCodeLogInfo.setLogPersonCompany(companyName);
				twoCodeLogInfo.setLogAction("待领");
				twoCodeLogInfo.setLogContext(this.dlregistNumber+"电梯放入待领区");
				flag = twoCodeLogInfo.save();
				if (flag) {
					result = "success";
				} else {
	...
	//	 int userid = userinfo.getId();
		 ...
		 try {
			...
			if(user != null){
				...
				twoCodeLogInfo.setLogPerson(userName);
				twoCodeLogInfo.setLogPersonCompany(companyName);
				twoCodeLogInfo.setLogAction("认领");
				twoCodeLogInfo.setLogContext(this.dlregistNumber2+"电梯被认领");
				flag = twoCodeLogInfo.save();
				if (flag) {
					result = "success";
				} else {
	...
	//	 int userid = userinfo.getId();
		 ...
		 try {
			...
			if(user != null){
				...
				twoCodeLogInfo.setLogPerson(userName);
				twoCodeLogInfo.setLogPersonCompany(companyName);
				twoCodeLogInfo.setLogAction("管理");
				twoCodeLogInfo.setLogContext("电梯管理列表");
				flag = twoCodeLogInfo.save();
				if (flag) {
					result = "success";
				} else {
	 ...
	 public View shemielevatorlist(int page, int rows){
	     ...
	     if(role==10 || role ==11){
	    	 ...
	    	 String sql ="select t.shemiFlag,t.id,t.registNumber,t.address,isnull(t.buildingName,'') as buildingName,t.unit,t.registCode,t.useNumber,t.area,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName  from TwoCodeElevatorInfo  t  where  t.area = ? and t.shemiFlag = 1";
	    	 items = ElevaltorInfoVO.findBySql(ElevaltorInfoVO.class, sql, new Object[]{companyArea}, "t.updateTime desc", rows, (page-1)*rows);	
	    	 }
	     }
	     } catch (ActiveRecordException e) {
	 ...
	 public View dailingelequery(ElevaltorInfoVO info,int page, int rows)throws Exception{
		 ...
		 if(role==2 || role==1){
		    if("".equals(info.getRegistCode())){ 
		    	 if("".equals(info.getUseNumber())){
		    	       conditions =" t.registNumber like  ?  and t.area like ? and  (t.registCode like ? or t.registCode is null) and (t.useNumber like ? or t.useNumber is null) and ((t.address like ? or t.buildingName like ?) or  (t.wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
				       conditionsSql=" registNumber like  ?  and area like ? and (registCode like ? or registCode is null) and (useNumber like ? or useNumber is null) and ((address like ? or buildingName like ?) or  (wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
				    
		    	 }
		    	 else{
		         conditions =" t.registNumber like  ?  and t.area like ? and  (t.registCode like ? or t.registCode is null) and t.useNumber like ? and ((t.address like ? or t.buildingName like ?) or  (t.wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
		         conditionsSql=" registNumber like  ?  and area like ? and (registCode like ? or registCode is null) and useNumber like ? and ((address like ? or buildingName like ?) or  (wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
		      }
		    }
		    else{
		    	 if("".equals(info.getUseNumber())){
		    		 conditions =" t.registNumber like  ?  and t.area like ? and  t.registCode like ? and (t.useNumber like ? or t.useNumber is null) and ((t.address like ? or t.buildingName like ?) or  (t.wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
				     conditionsSql=" registNumber like  ?  and area like ? and registCode like ?  and (useNumber like ? or useNumber is null) and ((address like ? or buildingName like ?) or  (wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
			    	  
		    	 }
		    	 else{
		    	 conditions =" t.registNumber like  ?  and t.area like ? and  t.registCode like ? and t.useNumber like ? and ((t.address like ? or t.buildingName like ?) or  (t.wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
			     conditionsSql=" registNumber like  ?  and area like ? and registCode like ?  and useNumber like ? and ((address like ? or buildingName like ?) or  (wgCompanyId in (select tcc.id from TwoCodeCompanyInfo tcc where   tcc.type ='使用' and tcc.companyName like ?)))";
		    	 }
		    }
		 } 
		 if(role==22 || role==23){
	...
	//	 result.put("qinspectDate2", gjQueryInfoVO.getQinspectDate2());
	 ...
	 public View lyztstatisticslist(int page, int rows){
		 ...
		 long total=0;
	//	 String sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and  te.companyid = t.id ) as tdetotal from  TwoCodeCompanyInfo t  where  t.type ='检验'";
		 ...
		 try {
			total  =DdElevaltorInfoVO.countBySql(DdElevaltorInfoVO.class,"select count(*) from TwoCodeCompanyInfo t  where  type ='检验'",null);
	 ...
	 public View lyztstatisticsquery(DdElevaltorInfoVO info,int page, int rows){
		 ...
		 if(!"".equals(areaName)){
			 ...
			 if(!"".equals(conditions2)){
				 if("".equals(conditions3))
			//	     sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and  te.companyid = t.id ) as tdetotal  from  TwoCodeCompanyInfo t  where  t.type ='检验' ";
					 sql="select t.id,t.companyName as ywCompanyName,(select sum(rcount) from TwoCodePasteReleaseTab where companyId =t.id) as issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and  te.companyid = t.id ) as tdetotal  from  TwoCodeCompanyInfo t  where  t.type ='检验' ";
					 
				else
			//		 sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and "+ conditions3+" and  te.companyid = t.id ) as tdetotal  from  TwoCodeCompanyInfo t  where  t.type ='检验' ";
			...
			//	   sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and  te.companyid = t.id ) as tdetotal  from  TwoCodeCompanyInfo t  where  t.type ='检验' ";
			 ...
			 conditionsSql ="select count(*) from TwoCodeCompanyInfo t  where  type ='检验' ";
	 ...
	 public View ztstatisticsquery(DdElevaltorInfoVO info,int page, int rows){
		 ...
		 if(!"".equals(conditions)){
			...
			//		sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and "+ conditions3+" and  te.companyid = t.id ) as tdetotal,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1  and  td.subTime2 >=dateadd(week,-1,DATEADD(week,DATEDIFF(week,0,getdate()),0)) and  td.subTime2 < DATEADD(week,-1,DATEADD(week,DATEDIFF(week,0,getdate()),7))  and  te.companyid = t.id ) as pretdetotal from  TwoCodeCompanyInfo t  where (t.type ='维保' or t.type ='检验') and "+conditions;
			 ...
			 conditionsSql ="select count(*) from TwoCodeCompanyInfo t  where (type ='维保' or type ='检验') and "+conditions;
		 }else{
			 if(!"".equals(conditions2)){
				 if("".equals(conditions3))
			//	     sql="select t.id,t.companyName as ywCompanyName,t.issueetotal,t.type,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1   and  te.companyid = t.id ) as tdetotal,(select count(td.registNumber) from  TwoCodeDdElevatorInfo  td left join  TwoCodeUserExtInfo te on   td.subPersonID = te.userid where   td.recordSate >1  and "+ conditions2+"  and  te.companyid = t.id ) as pretdetotal from  TwoCodeCompanyInfo t  where t.type ='维保' or t.type ='检验' ";
			 ...
			 conditionsSql ="select count(*) from TwoCodeCompanyInfo t  where type ='维保' or type ='检验' ";
	...
	//	 String sql ="select t1.area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area"; 
		 if("0".equals(cityName)){
		  sql ="select (CASE t1.area WHEN '锦江区' THEN 1  WHEN '青羊区' THEN 2 WHEN '金牛区' THEN 3 WHEN '武侯区' THEN 4 WHEN '成华区' THEN 5 WHEN '高新区' THEN 6 WHEN '天府新区' THEN 7 WHEN '龙泉驿区' THEN 8 WHEN '青白江区' THEN 9 WHEN '新都区' THEN 10 WHEN '温江区' THEN 11 WHEN '金堂县' THEN 12 WHEN '双流区' THEN 13 WHEN '郫都区' THEN 14 WHEN '大邑县' THEN 15 WHEN '蒲江县' THEN 16 WHEN '新津县' THEN 17 WHEN '都江堰市' THEN 18 WHEN '彭州市' THEN 19 WHEN '邛崃市' THEN 20 WHEN '崇州市' THEN 21 WHEN '简阳市' THEN 22 ELSE  0 END) as mrindex, t1.area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber   group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area order by mrindex "; 
		 }
		 else{
	...
	//	 String sql ="select t1.townshipStreets,(select companyName from twocodecompanyinfo where id =t1.townshipStreets ) as jdbCompanyName,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal from (select  t.townshipStreets,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.townshipStreets ) t1 left join TwoCodeReamarkEleByJdbView  rv  on t1.townshipStreets =  rv.townshipStreets left join TwoCodeCompanyinfo tc on  t1.townshipStreets =tc.id where tc.parea = ? "; 
	     ...
	     if(userInfoVO != null){
	    	 ...
	    	 CompanyInfoVO  wzCompanyInfo  =CompanyInfoVO.findFirstBySql(CompanyInfoVO.class, "select t.id from TwoCodeCompanyInfo t where t.companyName ='未知' and t.type='街道办' ", null);
	    	 int wzcompanyId =0;
	    	 if(wzCompanyInfo != null)
	    		 wzcompanyId = wzCompanyInfo.getId();   //未知街道办的id
		//   total =ElevaltorInfoVO.countBySql(ElevaltorInfoVO.class, "select count(1) from (select townshipStreets   from twocodeelevatorinfo group by townshipStreets) t left join TwoCodeCompanyinfo tc on  t.townshipStreets =tc.id where tc.parea = ? ", new Object[]{companyArea});
	 ...
	 public View systongjiquery2(String ratingDate){
		 ...
		 if(tjtime.substring(0,7).equals(ratingDate)){  //当前月要执行存储过程，其它月如果TwoCodeSystj没有，才执行存储过程
			 sql ="select (CASE t1.area WHEN '锦江区' THEN 1  WHEN '青羊区' THEN 2 WHEN '金牛区' THEN 3 WHEN '武侯区' THEN 4 WHEN '成华区' THEN 5 WHEN '高新区' THEN 6 WHEN '天府新区' THEN 7 WHEN '龙泉驿区' THEN 8 WHEN '青白江区' THEN 9 WHEN '新都区' THEN 10 WHEN '温江区' THEN 11 WHEN '金堂县' THEN 12 WHEN '双流区' THEN 13 WHEN '郫都区' THEN 14 WHEN '大邑县' THEN 15 WHEN '蒲江县' THEN 16 WHEN '新津县' THEN 17 WHEN '都江堰市' THEN 18 WHEN '彭州市' THEN 19 WHEN '邛崃市' THEN 20 WHEN '崇州市' THEN 21 WHEN '简阳市' THEN 22 ELSE  0 END) as mrindex,  t1.area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area order by mrindex";
		...
		// 打印  
	     ...
	     String SortValue =request.getParameter("order");  
	      ...
	      ratingDateMonthSpace =dogetMonthSpace(ratingDate,s); 
			if(ratingDateMonthSpace != 0){  //不是本月的查询从历史表中查询
				 System.out.print("非本月数据");
	...
	//	 sql ="select t.registNumber,t.address,t.buildingName,t.building,t.unit,tc.companyname as wgcompanyName,tc.telephone,t.area,y.endTime,tu.userName,tu.companyName as ywcompanyName,tu.contactPhone  from twocodeelevatorinfo  t   left join YwManagerInfo  y on  t.registNumber = y.registNumber left join tcuserinfo  tu on tu.userid = y.userid left join twocodecompanyinfo tc on tc.id = t.wgcompanyId where DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15  and t.dailingFlag = 0 and t.townshipStreets = "+townshipStreets;
	 ...
	 public View ncqetotallist2jdbByOrder( ElevaltorInfoVO info,int page, int rows,String sort,String order){
		 ...
		 if(wzcompanyId ==townshipStreets && townshipStreets > 0){   //未知街道办要只显示该区域内的未知街道办
...
//		 sql = "select t.area,count(t.registNumber) as etotal from  twocodeelevatorinfo  t  group by t.area  order by etotal desc" ;
	...
	//		 ActiveRecordBase.execute("exec proc_deleteElevatorinfo ?", new Object[] { registNumber });
	 ...
	 public View zxelevatorlistByOrder(int page, int rows,String sort,String order){
	     ...
	     if(role==2 || role ==1){ //系统管理员 TwoCodeCompanyInfo 
			 ...
			 total  =ZXElevaltorInfoVO.count(ZXElevaltorInfoVO.class, null, null);
	      	 String sql ="select t.shemiFlag,t.dailingFlag,t.id,t.area,t.registNumber,isnull(t.address,'') as address,isnull(t.buildingName,'') as buildingName,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,t.zzCompanyId,t.azCompanyId,t.wgCompanyId,t.ywCompanyId,t.zjCompanyId,t.jyCompanyId,t.townshipStreets,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,t.ischangInfo  from TwoCodeZhuXElevatorInfo  t   "; 
	 ...
	 public View queryZXByOrder(ElevaltorInfoVO info,int page, int rows,String sort,String order)throws Exception{
        ...
        //     conditions = " t.zjcompanyId = "+zjcompanyId2;
         ...
         if(!"".equals(conditions)){
          sql ="select t.shemiFlag,t.dailingFlag,t.id,t.area,t.registNumber,t.address,t.buildingName,t.registCode,t.useNumber,t.inspector,t.inspectDate,t.nextInspectDate,t.zzCompanyId,t.azCompanyId,t.wgCompanyId,t.ywCompanyId,t.zjCompanyId,t.jyCompanyId,t.townshipStreets,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.wgCompanyId and tc.type ='使用') as wgCompanyName, (select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.ywCompanyId and tc.type ='维保') as ywCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zzCompanyId and tc.type ='制造') as zzCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.azCompanyId and tc.type ='安装') as azCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.jyCompanyId and tc.type ='检验') as jyCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.zjCompanyId and tc.type ='质监') as zjCompanyName,(select tc.companyName from TwoCodeCompanyInfo tc where   tc.id =t.townshipStreets and tc.type ='街道办') as jdbCompanyName,t.ischangInfo  from TwoCodeZhuXElevatorInfo  t   where "+ conditions;
	 ...
	 */
     ...
     {   
		 ...
		 if("0".equals(cityName)){
			 ...
			 sql ="(select (CASE t1.area WHEN '锦江区' THEN 1  WHEN '青羊区' THEN 2 WHEN '金牛区' THEN 3 WHEN '武侯区' THEN 4 WHEN '成华区' THEN 5 WHEN '高新区' THEN 6 WHEN '天府新区' THEN 7 WHEN '龙泉驿区' THEN 8 WHEN '青白江区' THEN 9 WHEN '新都区' THEN 10 WHEN '温江区' THEN 11 WHEN '金堂县' THEN 12 WHEN '双流区' THEN 13 WHEN '郫都区' THEN 14 WHEN '大邑县' THEN 15 WHEN '蒲江县' THEN 16 WHEN '新津县' THEN 17 WHEN '都江堰市' THEN 18 WHEN '彭州市' THEN 19 WHEN '邛崃市' THEN 20 WHEN '崇州市' THEN 21 WHEN '简阳市' THEN 22 ELSE  0 END) as mrindex, (CASE t1.area WHEN '锦江区' THEN '锦江区'  WHEN '青羊区' THEN '青羊区' WHEN '金牛' THEN '金牛区' WHEN '武侯区' THEN '武侯区' WHEN '成华区' THEN '成华区' WHEN '高新区' THEN '高新区' WHEN '天府新区' THEN '天府新区' WHEN '龙泉驿' THEN '龙泉驿区' WHEN '青白江区' THEN '青白江区' WHEN '新都区' THEN '新都区' WHEN '温江区' THEN '温江区' WHEN '金堂县' THEN '金堂县' WHEN '双流县' THEN '双流县' WHEN '郫县' THEN '郫县' WHEN '大邑县' THEN '大邑县' WHEN '蒲江县' THEN '蒲江县' WHEN '新津县' THEN '新津县' WHEN '都江堰市' THEN '都江堰市' WHEN '彭州市' THEN '彭州市' WHEN '邛崃市' THEN '邛崃市' WHEN '崇州市' THEN '崇州市' WHEN '简阳市' THEN '简阳市' ELSE  t1.area END) as area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal,(case when tsrtotal > 0 then cast((undotsrtotal* 1./tsrtotal*100) as   numeric(5,2) ) else 0.00 end )as nutotal,(case when (etotal-setotal) > 0  then cast((ncqetotal* 1./(etotal - setotal)*100 ) as numeric(5,2)) else 0.00 end ) as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) UNION ALL (select 100 as mrindex,'合计' as area,sum(t1.etotal) as etotal,sum(t1.setotal) as setotal,sum(t1.ncqetotal) as ncqetotal,sum(rv.tsrtotal) as tsrtotal,sum(rv.undotsrtotal) as undotsrtotal,0.00 as nutotal,0.00 as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) order by mrindex";
			 items = SystjInfoVO.findBySql(SystjInfoVO.class, sql, null,null);			 
			 }
	//	    sql ="(select (CASE t1.area WHEN '锦江区' THEN 1  WHEN '青羊区' THEN 2 WHEN '金牛区' THEN 3 WHEN '武侯区' THEN 4 WHEN '成华区' THEN 5 WHEN '高新区' THEN 6 WHEN '天府新区' THEN 7 WHEN '龙泉驿区' THEN 8 WHEN '青白江区' THEN 9 WHEN '新都区' THEN 10 WHEN '温江区' THEN 11 WHEN '金堂县' THEN 12 WHEN '双流县' THEN 13 WHEN '郫县' THEN 14 WHEN '大邑县' THEN 15 WHEN '蒲江县' THEN 16 WHEN '新津县' THEN 17 WHEN '都江堰市' THEN 18 WHEN '彭州市' THEN 19 WHEN '邛崃市' THEN 20 WHEN '崇州市' THEN 21 WHEN '简阳市' THEN 22 ELSE  0 END) as mrindex, (CASE t1.area WHEN '锦江区' THEN '锦江区'  WHEN '青羊区' THEN '青羊区' WHEN '金牛' THEN '金牛区' WHEN '武侯区' THEN '武侯区' WHEN '成华区' THEN '成华区' WHEN '高新区' THEN '高新区' WHEN '天府新区' THEN '天府新区' WHEN '龙泉驿' THEN '龙泉驿区' WHEN '青白江区' THEN '青白江区' WHEN '新都区' THEN '新都区' WHEN '温江区' THEN '温江区' WHEN '金堂县' THEN '金堂县' WHEN '双流县' THEN '双流县' WHEN '郫县' THEN '郫县' WHEN '大邑县' THEN '大邑县' WHEN '蒲江县' THEN '蒲江县' WHEN '新津县' THEN '新津县' WHEN '都江堰市' THEN '都江堰市' WHEN '彭州市' THEN '彭州市' WHEN '邛崃市' THEN '邛崃市' WHEN '崇州市' THEN '崇州市' WHEN '简阳市' THEN '简阳市' ELSE  t1.area END) as area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal,(case when tsrtotal > 0 then cast((undotsrtotal* 1./tsrtotal*100) as   numeric(5,2) ) else 0.00 end )as nutotal,(case when (etotal-setotal) > 0  then cast((ncqetotal* 1./(etotal - setotal)*100 ) as numeric(5,2)) else 0.00 end ) as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) UNION ALL (select 100 as mrindex,'合计' as area,sum(t1.etotal) as etotal,sum(t1.setotal) as setotal,sum(t1.ncqetotal) as ncqetotal,sum(rv.tsrtotal) as tsrtotal,sum(rv.undotsrtotal) as undotsrtotal,0.00 as nutotal,0.00 as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) order by mrindex";
			else {
			sql ="(select mrindex,area,etotal,setotal,ncqetotal,tsrtotal,undotsrtotal,(case when tsrtotal > 0 then cast((undotsrtotal* 1./tsrtotal*100) as   numeric(5,2) ) else 0.00 end )as nutotal,(case when (etotal-setotal) > 0  then cast((ncqetotal* 1./(etotal - setotal)*100 ) as numeric(5,2)) else 0.00 end ) as cql from TwoCodeSystj where ratingDate = ? ) UNION ALL (select 100 as mrindex,'合计' as area,sum(t1.etotal) as etotal,sum(t1.setotal) as setotal,sum(t1.ncqetotal) as ncqetotal,sum(rv.tsrtotal) as tsrtotal,sum(rv.undotsrtotal) as undotsrtotal,0.00 as nutotal,0.00 as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) order by mrindex";
			...
			items = SystjInfoVO.findBySql(SystjInfoVO.class, sql, null,null);
			 ...
			 String[] hearders = new String[] {"行政区划","电梯数量", "停用数量", "维保超期数量", "投诉数量", "投诉未处理", "投诉未处理率","维保超期率"};//表头数组 
			 SimpleDateFormat timeFormat = new SimpleDateFormat("yyyyMMddHHmmss");  
			 String filename = "运行统计表_"+timeFormat.format(new Date())+".xls";  
			 response.setContentType("application/ms-excel;charset=UTF-8");  
			 response.setHeader("Content-Disposition", "attachment;filename="  
			            .concat(String.valueOf(URLEncoder.encode(filename, "UTF-8"))));
			 OutputStream out = response.getOutputStream(); 
     ...
     {   
		 ...
		 List<SystjInfoVO> items = new ArrayList<SystjInfoVO>();
		 String sql ="";
	//   String sql ="select (CASE t1.area WHEN '锦江' THEN 1  WHEN '青羊' THEN 2 WHEN '金牛' THEN 3 WHEN '武侯' THEN 4 WHEN '成华' THEN 5 WHEN '高新' THEN 6 WHEN '天府新' THEN 7 WHEN '龙泉驿' THEN 8 WHEN '青白江' THEN 9 WHEN '新都' THEN 10 WHEN '温江' THEN 11 WHEN '金堂' THEN 12 WHEN '双流' THEN 13 WHEN '郫县' THEN 14 WHEN '大邑' THEN 15 WHEN '蒲江' THEN 16 WHEN '新津' THEN 17 WHEN '都江堰' THEN 18 WHEN '彭州' THEN 19 WHEN '邛崃' THEN 20 WHEN '崇州' THEN 21 WHEN '简阳' THEN 22 ELSE  0 END) as mrindex, (CASE t1.area WHEN '锦江' THEN '锦江区'  WHEN '青羊' THEN '青羊区' WHEN '金牛' THEN '金牛区' WHEN '武侯' THEN '武侯区' WHEN '成华' THEN '成华区' WHEN '高新' THEN '高新区' WHEN '天府新' THEN '天府新区' WHEN '龙泉驿' THEN '龙泉驿区' WHEN '青白江' THEN '青白江区' WHEN '新都' THEN '新都区' WHEN '温江' THEN '温江区' WHEN '金堂' THEN '金堂县' WHEN '双流' THEN '双流区' WHEN '郫县' THEN '郫县' WHEN '大邑' THEN '大邑县' WHEN '蒲江' THEN '蒲江县' WHEN '新津' THEN '新津县' WHEN '都江堰' THEN '都江堰市' WHEN '彭州' THEN '彭州市' WHEN '邛崃' THEN '邛崃市' WHEN '崇州' THEN '崇州市' WHEN '简阳' THEN '简阳市' ELSE  t1.area END) as area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal,cast((undotsrtotal* 1./tsrtotal*100) as   numeric(5,2) )as nutotal,cast((ncqetotal* 1./(etotal - setotal)*100 ) as numeric(5,2)) as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area order by mrindex  "; 
		 if("0".equals(cityName)){
		    sql ="(select (CASE t1.area WHEN '锦江区' THEN 1  WHEN '青羊区' THEN 2 WHEN '金牛区' THEN 3 WHEN '武侯区' THEN 4 WHEN '成华区' THEN 5 WHEN '高新区' THEN 6 WHEN '天府新区' THEN 7 WHEN '龙泉驿区' THEN 8 WHEN '青白江区' THEN 9 WHEN '新都区' THEN 10 WHEN '温江区' THEN 11 WHEN '金堂县' THEN 12 WHEN '双流区' THEN 13 WHEN '郫都区' THEN 14 WHEN '大邑县' THEN 15 WHEN '蒲江县' THEN 16 WHEN '新津县' THEN 17 WHEN '都江堰市' THEN 18 WHEN '彭州市' THEN 19 WHEN '邛崃市' THEN 20 WHEN '崇州市' THEN 21 WHEN '简阳市' THEN 22 ELSE  0 END) as mrindex, (CASE t1.area WHEN '锦江区' THEN '锦江区'  WHEN '青羊区' THEN '青羊区' WHEN '金牛' THEN '金牛区' WHEN '武侯区' THEN '武侯区' WHEN '成华区' THEN '成华区' WHEN '高新区' THEN '高新区' WHEN '天府新区' THEN '天府新区' WHEN '龙泉驿' THEN '龙泉驿区' WHEN '青白江区' THEN '青白江区' WHEN '新都区' THEN '新都区' WHEN '温江区' THEN '温江区' WHEN '金堂县' THEN '金堂县' WHEN '双流县' THEN '双流县' WHEN '郫县' THEN '郫县' WHEN '大邑县' THEN '大邑县' WHEN '蒲江县' THEN '蒲江县' WHEN '新津县' THEN '新津县' WHEN '都江堰市' THEN '都江堰市' WHEN '彭州市' THEN '彭州市' WHEN '邛崃市' THEN '邛崃市' WHEN '崇州市' THEN '崇州市' WHEN '简阳市' THEN '简阳市' ELSE  t1.area END) as area,t1.etotal,t1.setotal,t1.ncqetotal,isnull(rv.tsrtotal,0) as tsrtotal,isnull(rv.undotsrtotal,0) as  undotsrtotal,(case when tsrtotal > 0 then cast((undotsrtotal* 1./tsrtotal*100) as   numeric(5,2) ) else 0.00 end )as nutotal,(case when (etotal-setotal) > 0  then cast((ncqetotal* 1./(etotal - setotal)*100 ) as numeric(5,2)) else 0.00 end ) as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) UNION ALL (select 100 as mrindex,'合计' as area,sum(t1.etotal) as etotal,sum(t1.setotal) as setotal,sum(t1.ncqetotal) as ncqetotal,sum(rv.tsrtotal) as tsrtotal,sum(rv.undotsrtotal) as undotsrtotal,0.00 as nutotal,0.00 as cql from (select  t.area,count(t.registNumber) as etotal,sum(CASE WHEN t.dailingFlag = 1 THEN 1 ELSE 0 END) as setotal,sum(CASE WHEN  (DATEDIFF(dd,convert(datetime,y.endTime,120),getdate()) > 15 and t.dailingFlag = 0) THEN 1 ELSE 0 END) as ncqetotal from twocodeelevatorinfo t   left join YwManagerInfo  y on t.registNumber = y.registNumber group by t.area ) t1 left join TwoCodeReamarkEleView  rv  on t1.area =  rv.area ) order by mrindex";
		 }
		 else{
  	    	...
  	    	if(num > 0){   //从记录更新成功，更新主记录
			      ...
			      if(pInfo != null){
			    	  sql ="select sum(inspectElevatorTimes) as inspectElevatorTimes,sum(inspectElevatorTimes2) as inspectElevatorTimes2,sum(acceptInspElevatorTimes) as acceptInspElevatorTimes,sum(maintenSceneInfoTimes) as maintenSceneInfoTimes from TwoCodeYwQuaCredRating where ywCompanyID =?   and ratingDate = ? and ratingType = ?";
			    		...
			    		int acceptInspElevatorTimes=0;  //接受监督检查情况
			    		...
			    		int maintenBusinessTimes = 0;
	   			        if(cInfo != null){	
	   			    		...
	   			    		acceptInspElevatorTimes=cInfo.getAcceptInspElevatorTimes();  //接受监督检查情况
		    		...
		    		int maintenBusinessTimes = 0;
	   			        if(cInfo != null){	
	   			    		//处理和区县市质监共有打分的部分
	   			    		 ...
	   			    		 inspectElevatorTimes2 =cInfo.getInspectElevatorTimes2();   
	   			    		 acceptInspElevatorTimes=cInfo.getAcceptInspElevatorTimes();  //接受监督检查情况
	   			    		 maintenSceneInfoTimes =cInfo.getMaintenSceneInfoTimes();  //维保现场
	   			    		 complaintsEventsTimes=cInfo.getComplaintsEventsTimes();
	   			    		 complaintsEventsTimes2=cInfo.getComplaintsEventsTimes2();
	   			    		 maintenBusinessTimes = cInfo.getMaintenBusinessTimes();
	   			    	}
			      ...
			      if(pInfo != null){
	  			    ...
	  			    //		pInfo = YwQuaCredRating.findFirstBySql(YwQuaCredRating.class, "select complaintsEventssj,maintenBusinesssj,honestsj,punishmentsj from TwoCodeYwQuaCredRating where ywCompanyID =?  and ratingDate = ? and ratingType = ?", new Object[]{info.getYwCompanyID(),info.getRatingDate(),0});
	  			    	...
	  			    	//更新主记录的次数
	  			           ...
	  			           //处理区（市）县质监单独打分的部分（没有单独打分的，所以不需要处理）
	  			    	    if(pInfo.update()){
				    			result = "success";
				    		}
			          }
 	...
 	public String ywSysSetingshyxh(int ywCompanyID,String ratingDate,YwQuaCredRatingHyxh info){
		...
		UserExtInfo  info2 =null;
 		...
 		try {
	    	...
	    	if(info2 != null){
	    		userName = info2.getUserName();
	    	//	companyid = info2.getCompanyid();	//行业协会要找市质监的companyId
	    		companyinfo = CompanyInfoVO.findFirstBySql(CompanyInfoVO.class, "select id  from TwoCodeCompanyInfo where type = ?", new Object[]{"市质监"});
	    		if(companyinfo != null){
  	    		   ...
  	    		   //从记录添加成功，更新主记录
  	    			  ...
  	    			  if(pInfo != null){
  	    				 String sql= "";
	 			    		...
	 			    		pInfo.setEnterpriseRecordsj(info.getEnterpriseRecordsj());
	 			    		pInfo.setEnterpriseRecordbz(info.getEnterpriseRecordbz());
	 			    		if(pInfo.update()){ //评分记录更新成功，更新评分常量
 	 			    	//		int unum = YwQuaCredRatingConstant.updateAll(YwQuaCredRatingConstant.class, "officeSpace =?,fixedTelOnDuty=?", new Object[]{info.getOfficeSpace(),info.getFixedTelOnDuty()}, "ywCompanyID = ? ", new Object[]{ywCompanyID} ); 
	 			    			int unum = YwQuaCredRatingConstant.updateAll(YwQuaCredRatingConstant.class, "officeSpace =?,officeSpacesj=?,officeSpacebz=?,fixedTelOnDuty=?,fixedTelOnDutysj=?,fixedTelOnDutybz=?", new Object[]{info.getOfficeSpace(),info.getOfficeSpacesj(),info.getOfficeSpacebz(),info.getFixedTelOnDuty(),info.getFixedTelOnDutysj(),info.getFixedTelOnDutybz()}, "ywCompanyID = ? ", new Object[]{ywCompanyID} ); 
	...
	//	String userName =userinfo.getLoginName();
 	...
 	public View ywsyssetings2ByComIdRatDate(int ywCompanyID,String ratingDate){
		 ...
		 String userName ="";
	 	 ...
	 	 String sql = "";
	 	 Map<String, Object> result = new HashMap<String, Object>();
		 if((role == 22 || role ==23) || (role == 30 || role ==31)){ //市质监 ,行业协会
			 try {
 	...
 	public View ywSysSetingsItemDetail(String ItemName,int ywCompanyID,String ratingDate){
 		...
 		//	 sql = "select '办公面积' as itemName, officeSpace as itemContent,officeSpacebz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 
 			
 			 sql = "select '办公面积' as itemName, convert(varchar,officeSpace)+'㎡' as itemContent,officeSpacebz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 
 			
 		 }
 		 else if("headQuarters".equals(ItemName)){
 	//		 sql = "select '总部所在地' as itemName, (case when headQuarters =0 then '成都' else '外地' end )as itemContent,headQuartersbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
 			
 			 sql = "select '总部所在地' as itemName, (case when headQuarters =0 then '成都' else '外地' end )as itemContent,headQuartersbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
 			
 		 }
 		 else if("maintenanceEleCount".equals(ItemName)){
 		//	 sql = "select '维保电梯数量' as itemName, maintenanceEleCount as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			 sql = "select '维保电梯数量' as itemName, convert(varchar,maintenanceEleCount)+'台' as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 			
 		 }
 		else if("avgmaintenanceEleCount".equals(ItemName)){
 	 	//	 sql = "select '人均维保数量' as itemName, avgmaintenanceEleCount as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			sql = "select '人均维保数量' as itemName, avgmaintenanceEleCount as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 			 	 
 		 }
 		else if("fixedTelOnDuty".equals(ItemName)){
	 	//	 sql = "select '值班固定电话' as itemName, fixedTelOnDuty as itemContent,fixedTelOnDutybz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 			sql = "select '值班固定电话' as itemName, fixedTelOnDuty as itemContent,fixedTelOnDutybz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 		}
 		else if("telOnDutyunattendedTimes".equals(ItemName)){
	 	//	 sql = "select '电话值守无人接听' as itemName, telOnDutyunattendedTimes as itemContent,telOnDutyunattendedbz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 			sql = "select '电话值守无人接听' as itemName,  convert(varchar,telOnDutyunattendedTimes)+'次' as itemContent,telOnDutyunattendedbz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 		}
 		else if("enterpriseChangeTimes".equals(ItemName)){
	 	//	 sql = "select '维保企业变更' as itemName, enterpriseChangeTimes as itemContent,enterpriseChangebz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 			sql = "select '维保企业变更' as itemName, convert(varchar,enterpriseChangeTimes)+'宗' as itemContent,enterpriseChangebz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 		}
 		else if("enterpriseRecord".equals(ItemName)){
	 //		 sql = "select '企业备案' as itemName, (case when headQuarters =0 then '有' else '无' end ) as itemContent,enterpriseRecordbz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 			sql = "select '企业备案' as itemName, (case when headQuarters =0 then '有' else '无' end ) as itemContent,enterpriseRecordbz as itembz, (select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监'  ";	 			 	 
 			
 		}
 		 else if("infoComRate".equals(ItemName)){
 	//		 sql = "select '信息完整率' as itemName, infoComRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			 sql = "select '信息完整率' as itemName, infoComRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 			
 		 }
 		else if("sweepCodeRate".equals(ItemName)){
 	 	//	 sql = "select '扫码维保率' as itemName, sweepCodeRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			sql = "select '扫码维保率' as itemName, sweepCodeRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 				 	 
 		 }
 		else if("sweepCodeInTimeRate".equals(ItemName)){
	 	//	 sql = "select '按时扫码维保率' as itemName, sweepCodeInTimeRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			sql = "select '按时扫码维保率' as itemName, sweepCodeInTimeRate as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 				 
		 }
 		else if("alarmDealwith".equals(ItemName)){
	 	//	 sql = "select '平台报警处理情况' as itemName, alarmDealwith as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			sql = "select '平台报警处理情况' as itemName, convert(varchar,alarmDealwith)+'台' as itemContent,'' as itembz, '安全服务平台' as ratingUserName,'安全服务平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 				 	 
		 }
 		else if("regularInspectionTimes".equals(ItemName)){
 		//	 sql = "select '电梯定期检验情况' as itemName, regularInspectionTimes as itemContent,regularInspectionbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '检验' ";	 
 			
 			sql = "select '电梯定期检验情况' as itemName, convert(varchar,regularInspectionTimes)+'台' as itemContent,regularInspectionbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '检验' ";	 
 			
 			
 		}
 		else if("inspectElevatorTimes".equals(ItemName)){
		//	 sql = "select '在用电梯监督抽查情况' as itemName, (convert(varchar,inspectElevatorTimes)+'严重隐患'+';'+convert(varchar,inspectElevatorTimes2)+'回路短接') as itemContent,inspectElevatorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
 			sql = "select '在用电梯监督抽查情况' as itemName, (convert(varchar,inspectElevatorTimes)+'台:严重隐患<br>'+convert(varchar,inspectElevatorTimes2)+'台:回路短接') as itemContent,inspectElevatorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
 		}
 		else if("acceptInspElevatorTimes".equals(ItemName)){
		//	 sql = "select '接受监督检查情况' as itemName, acceptInspElevatorTimes as itemContent,acceptInspElevatorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
 			sql = "select '接受监督检查情况' as itemName, convert(varchar,acceptInspElevatorTimes)+'次' as itemContent,acceptInspElevatorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
 		}
 		else if("maintenSceneInfoTimes".equals(ItemName)){
		//	 sql = "select '维保现场防护情况' as itemName, maintenSceneInfoTimes as itemContent,maintenSceneInfobz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
 			sql = "select '维保现场防护情况' as itemName, convert(varchar,maintenSceneInfoTimes)+'次' as itemContent,maintenSceneInfobz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '检验' or tc.type= '质监') ";	 
 			
         }
 		else if("malignantEventsTimes".equals(ItemName)){
		//	 sql = "select '因维保原因发生安全事故重大社会影响事件' as itemName, (convert(varchar,malignantEventsTimes)+'一般事故'+convert(varchar,malignantEventsTimes2)+'较大事故 '+convert(varchar,malignantEventsTimes3)+'重大事故' )as itemContent,malignantEventsbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and  tc.type= '市质监'  ";	 
 			
 			sql = "select '因维保原因发生安全事故重大社会影响事件' as itemName, (convert(varchar,malignantEventsTimes)+'宗:一般事故<br>'+convert(varchar,malignantEventsTimes2)+'宗:较大事故<br>'+convert(varchar,malignantEventsTimes3)+'宗:重大事故') as itemContent,malignantEventsbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and  tc.type= '市质监'  ";	 
 			
 		}
 		else if("complaintsEventsTimes".equals(ItemName)){
		//	 sql = "select '因维保质量而引发的投诉' as itemName, (convert(varchar,complaintsEventsTimes)+'一般投诉 '+convert(varchar,complaintsEventsTimes2)+'连续投诉 ' )as itemContent,complaintsEventsbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 			sql = "select '因维保质量而引发的投诉' as itemName, (convert(varchar,complaintsEventsTimes)+'宗:一般投诉<br> '+convert(varchar,complaintsEventsTimes2)+'宗:连续投诉 ' )as itemContent,complaintsEventsbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 		}
 		else if("maintenBusinessTimes".equals(ItemName)){
		//	 sql = "select '维保业务管理' as itemName, maintenBusinessTimes as itemContent,maintenBusinessbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 			sql = "select '维保业务管理' as itemName, convert(varchar,maintenBusinessTimes)+'宗' as itemContent,maintenBusinessbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 		}
 		else if("honestTimes".equals(ItemName)){
		//	 sql = "select '诚实守信情况' as itemName, honestTimes as itemContent,honestbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 			sql = "select '诚实守信情况' as itemName, convert(varchar,honestTimes)+'宗' as itemContent,honestbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 		}
 		else if("punishmentTimes".equals(ItemName)){
		//	 sql = "select '行政查处及处罚情况' as itemName, (convert(varchar,punishmentTimes)+'维保管理及维保质量被整改'+convert(varchar,punishmentTimes2)+'被下达监察意见过通报 '+convert(varchar,punishmentTimes3)+'连续被下达监察指令或通报2次及以上'+convert(varchar,punishmentTimes4)+'违反法律、法规及技术规范' )as itemContent,punishmentbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 			sql = "select '行政查处及处罚情况' as itemName, (convert(varchar,punishmentTimes)+'宗:维保管理及维保质量被整改<br>'+convert(varchar,punishmentTimes2)+'宗:被下达监察意见过通报<br> '+convert(varchar,punishmentTimes3)+'宗:连续被下达监察指令或通报2次及以上<br>'+convert(varchar,punishmentTimes4)+'宗:违反法律、法规及技术规范' )as itemContent,punishmentbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and ( tc.type= '市质监' or tc.type= '质监') ";	 
 			
 		}
 		else if("firstRescueTimes".equals(ItemName)){
	 	//	 sql = "select '一级救援情况' as itemName, firstRescueTimes as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	 		 	 
 			
 			sql = "select '一级救援情况' as itemName, convert(varchar,firstRescueTimes)+'次' as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	 		 	 
 			
 		}
 		else if("secondRescueTimes".equals(ItemName)){
	 //		 sql = "select '二级救援情况' as itemName, (convert(varchar,secondRescueTimes)+'未实施'+';'+convert(varchar,secondRescueTimes2)+'实施)as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
 			
 			sql = "select '二级救援情况' as itemName, (convert(varchar,secondRescueTimes)+'次:未实施<br>'+convert(varchar,secondRescueTimes2)+'次:实施')as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
 				 
		 }
 		 else if("secondRescuePoint".equals(ItemName)){
 	//		 sql = "select '参与二级救援布点' as itemName, secondRescuePoint as itemContent,secondRescuePointbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
 			
 			 sql = "select '参与二级救援布点' as itemName, convert(varchar,secondRescuePoint)+'个' as itemContent,secondRescuePointbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
 			
 		 }
 		else if("rescueResponseTimes".equals(ItemName)){
	 //		 sql = "select '应急救援响应情况' as itemName, rescueResponseTimes as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	 		 	 
 			
 			sql = "select '应急救援响应情况' as itemName, convert(varchar,rescueResponseTimes)+'次' as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	 		 	 
 			
 		}
		else if("tiringPeopleTimes".equals(ItemName)){
	 	//	 sql = "select '因维保质量引发困人故障' as itemName, tiringPeopleTimes as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =0  ";	 	
			
			sql = "select '因维保质量引发困人故障' as itemName, convert(varchar,tiringPeopleTimes)+'宗' as itemContent,'' as itembz, '96933平台' as ratingUserName,'96933平台' as ratingCompanyName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =0  ";	 	
				 	 
		 }
		 else if("positiveEnergyTimes".equals(ItemName)){
 	//		 sql = "select '献计献策、举报违法违规、行业潜规则' as itemName, positiveEnergyTimes as itemContent,positiveEnergybz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			 
			 sql = "select '献计献策、举报违法违规、行业潜规则' as itemName, convert(varchar,positiveEnergyTimes)+'宗' as itemContent,positiveEnergybz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
			 
		 }
		 else if("expertsSuggestionTimes".equals(ItemName)){
 	//		 sql = "select '提供专家及技术支持，参与故障及事故调查处理' as itemName, expertsSuggestionTimes as itemContent,expertsSuggestionbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			
			 sql = "select '提供专家及技术支持，参与故障及事故调查处理' as itemName, convert(varchar,expertsSuggestionTimes)+'宗' as itemContent,expertsSuggestionbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
			
		 }
		 else if("positiveWork".equals(ItemName)){
 		//	 sql = "select '积极承接监管部门指定电梯维保' as itemName, positiveWork as itemContent,positiveWorkbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			
			 sql = "select '积极承接监管部门指定电梯维保' as itemName, convert(varchar,positiveWork)+'台' as itemContent,positiveWorkbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
			 
		 }
		 else if("remoteMonitor".equals(ItemName)){
 	//		 sql = "select '采用远程监控、机器人保安' as itemName, remoteMonitor as itemContent,remoteMonitorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			
			 sql = "select '采用远程监控、机器人保安' as itemName, convert(varchar,remoteMonitor)+'台' as itemContent,remoteMonitorbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
			 
		 }
		 else if("elevatorInsurance".equals(ItemName)){
 		//	 sql = "select '购买电梯责任保险' as itemName, elevatorInsurance as itemContent,elevatorInsurancebz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			 
			 sql = "select '购买电梯责任保险' as itemName, convert(varchar,elevatorInsurance)+'台' as itemContent,elevatorInsurancebz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
			
		 }
		 else if("techinnovation".equals(ItemName)){
 		//	 sql = "select '其他开展技术创新及工作创新' as itemName, (convert(varchar,techinnovationTimes)+'1分'+convert(varchar,techinnovationTimes2)+'2分'+convert(varchar,techinnovationTimes3)+'3分'+convert(varchar,techinnovationTimes4)+'4分'+convert(varchar,techinnovationTimes5)+'5分') as itemContent,secondRescuePointbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=substring(CONVERT(varchar(12), getdate(), 120),0,8) and ratingType =1 and tc.type= '市质监' ";	 	
			 
			 sql = "select '其他开展技术创新及工作创新' as itemName, (convert(varchar,techinnovationTimes)+'项:1分<br>'+convert(varchar,techinnovationTimes2)+'项:2分<br>'+convert(varchar,techinnovationTimes3)+'项:3分<br>'+convert(varchar,techinnovationTimes4)+'项:4分<br>'+convert(varchar,techinnovationTimes5)+'项:5分') as itemContent,techinnovationbz as itembz,(select companyName from twoCodeCompanyinfo where id = ratingCompanyId) as ratingCompanyName, ratingUserName,CONVERT(varchar(100),detailratingDate, 120) as detailratingDate  from TwoCodeYwQuaCredRating left join TwoCodeCompanyInfo tc on TwoCodeYwQuaCredRating.ratingCompanyId =tc.id where ywCompanyID = ?  and  ratingDate=? and ratingType =1 and tc.type= '市质监' ";	 	
 	...
 	public View kpCompanyinfo(){
		List<CompanyInfo> items =null;
		try {	
			 items = CompanyInfo.findBySql(CompanyInfo.class,"select id,companyName from TwoCodeCompanyInfo where type = '质监' or type ='检验' or type='市质监' order by type desc ", null);	
			} catch (ActiveRecordException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
 	...
 	public View queryKpDetailinfoList(YwQuaCredRatingVO info){
 		...
 		try {	
 		  ...
 		  if("市质监".equals(ratingCompanyIdType)){	
 		 ...
 		 if("质监".equals(ratingCompanyIdType)){	
 		...
 		if("检验".equals(ratingCompanyIdType)){
	...
	public View kpinfoListMul(){
		...
		try {	
			if(role == 30 || role ==31)
			items = YwKaoPingInfo.findBySql(YwKaoPingInfo.class,"select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate,(officeSpacesj+headQuarterssj+fixedTelOnDutysj+telOnDutyunattendedsj+enterpriseChangesj+enterpriseRecordsj-100)   as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where  tc.type = '市质监' and ratingType = 1 and t.ratingDate = \'"+s+"\'", null);	
			if(role == 10 || role ==11)
			items = YwKaoPingInfo.findBySql(YwKaoPingInfo.class,"select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate,(inspectElevatorsj+acceptInspElevatorsj+maintenSceneInfosj+complaintsEventssj+maintenBusinesssj+honestsj+punishmentsj -300)   as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where  tc.type = '质监' and ratingType = 1 and t.ratingDate = \'"+s+"\'", null);	
			if(role == 16 || role ==17)
			items = YwKaoPingInfo.findBySql(YwKaoPingInfo.class,"select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate,(regularInspectionsj+inspectElevatorsj+acceptInspElevatorsj+maintenSceneInfosj-200)   as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where  tc.type = '检验' and ratingType = 1 and t.ratingDate = \'"+s+"\'", null);	
...
public View queryKpinfoListMul(YwKaoPingInfoVO info){
		...
		String sql = "";
		if(role == 30 || role ==31)
		    sql = "select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate, (officeSpacesj+headQuarterssj+fixedTelOnDutysj+telOnDutyunattendedsj+enterpriseChangesj+enterpriseRecordsj -100 ) as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where ratingType = 1 and tc.type = '市质监'";
		if(role == 10 || role ==11)
			sql = "select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate, (select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate,(inspectElevatorsj+acceptInspElevatorsj+maintenSceneInfosj+complaintsEventssj+maintenBusinesssj+honestsj+punishmentsj -300)   as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where ratingType = 1 and tc.type = '质监'";
		if(role == 16 || role ==17)
			sql = "select t.id,t.ratingCompanyId,t.ywCompanyID,t.ratingDate,tc.type,(select companyName from twocodeCompanyInfo where id =t.ratingCompanyId ) as ratingCompanyName,(select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate, (select companyName from twocodeCompanyInfo where id =t.ywCompanyID ) as ywCompanyName,ratingUserName,detailratingDate,(regularInspectionsj+inspectElevatorsj+acceptInspElevatorsj+maintenSceneInfosj-200)  as scoreTotal  from TwoCodeYwQuaCredRating  t left join  twocodecompanyinfo tc on t.ratingCompanyId = tc.id  where ratingType = 1 and tc.type = '检验'";
 ...
 //		picFile = req.getFile("officeSpaceImg");
 	  ...
 	  if (!file.exists()) {
			if (!file.mkdirs()) {
				System.out.println("GlobalConfig类,产生二维码上传目录错误");
			}
		}
	...
	//	 JSONObject obj = new JSONObject();
		 String sql ="";
		 try {
		 sql ="select top 200 count(*) as etotal,tc.companyName from TwoCodeCompanyInfo tc left join TwoCodeElevatorInfo t on tc.id = t.ywcompanyId where tc.type = '维保' group by tc.companyName  order by etotal desc"; 
 	...
 	public View cseletjMrEcharts(){
		 ...
		 String sql ="";
		 try {
		 sql ="select top 200 count(*) as etotal,tc.companyName from TwoCodeCompanyInfo tc left join TwoCodeElevatorInfo t on tc.id = t.wgcompanyId where tc.type = '使用' group by tc.companyName  order by etotal desc"; 
		 items =CsEchartsElevaltorInfoVO.findBySql(CsEchartsElevaltorInfoVO.class, sql); 
		} catch (ActiveRecordException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public View cseletjEllipseEcharts(){
		 ...
		 try {
	//	 sql ="select top 1 count(*) as companyTotal from  TwoCodeCompanyInfo where  type = '维保' union select top 1 count(*) as companyTotal from YwManagerInfo group by ywcompanyId"; 
        sql =" select top 1 COUNT(*) as companyTotal from  TwoCodeCompanyInfo where  type = '使用'  ";
		 items =CsEchartsYwInfoVO.findBySql(CsEchartsYwInfoVO.class, sql); 
		} catch (ActiveRecordException e) {
...
//			}
	 ...
	 public String daoruJc(Workbook workBook) {
		//获取一个sheet
			...
			Row rowFirst = sheet.getRow(0);
			//int lastCellNum = rowFirst.getLastCellNum();
			System.out.println(lastRowNum + "行数据");
			 //定义map集合
			 Map<String, Object> result = new HashMap<String, Object>();
			 //识别码已经存在的数据集合
			 //List<DdElevaltorInfo> shibieCodes = new ArrayList<DdElevaltorInfo>();
	 ...
	 public String plModify(ElevaltorInfoVO elevaltorInfoVo,String ywCompanyTel,String wgCompanyTel,String ids) {
		 ...
		 try {
			 ...
			 if(!"".equals(wgCompanyName) && wgCompanyName != null) {
				 CompanyInfo companyInfo = CompanyInfo.findFirst(CompanyInfo.class, "companyName=? and type=?", new Object[] {wgCompanyName,"使用"});
				 if(companyInfo == null) {
					 return "使用单位不存在请重新填写";
				 }
			 ...
			 if(!"".equals(ywCompanyName) && ywCompanyName != null) {
				 CompanyInfo companyInfo = CompanyInfo.findFirst(CompanyInfo.class, "companyName=? and type=?", new Object[] {ywCompanyName,"维保"});
				 if(companyInfo == null) {
					 return "维保单位不存在请重新填写";
				 }
